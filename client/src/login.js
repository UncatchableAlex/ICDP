...
export function LoginPage() {
  ...
  const postNewUser = () => {
    ...
    if (pwState.val.length < 8) {
      pwStateSet({
        helperText: " too short (8 characters min)",
        val: pwState.val,
        color: "error",
      });
      hasProblems = true;
    ...
    if (!normalUserName) {
      unStateSet({
        helperText: " no weird characters",
        val: unState.val,
        color: "error",
      });
      hasProblems = true;
  ...
  const loginButtons = (
    <Stack direction="row" spacing={2} alignItems="center">
      ...
      <Button
        variant="outlined"
        startIcon={<Person4Outlined />}
        onClick={() => hideEmailSet(false)}
      >
        New User
      </Button>
    </Stack>
  ...
  const newUserButtons = (
    <Stack direction="row" spacing={2} alignItems="center">
      <Button
        variant="outlined"
        startIcon={<KeyboardDoubleArrowLeftOutlined />}
